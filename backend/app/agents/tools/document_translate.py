"""`tarjima_qil` — translate one document, keeping the original next to it.

This file is only the *tool wrapper*: it resolves which document is meant and
turns the result into a `ToolResult`. The translation itself (paragraph
alignment, term rule, cache) lives in `services/translation.py`, the exact same
code path the "Tarjima" button uses via `POST /documents/{id}/translate`.

Open to every role — the restriction lives in the document layer: by title only
visible documents are searched (a confidential order is not even reported as
existing), by id an invisible document answers a plain "Ruxsat yo'q".
"""

from __future__ import annotations

from sqlalchemy.orm import Session

from app.agents.registry import ALL_ROLES, Tool, ToolResult, register
from app.models import Document, User
from app.services import documents as documents_service
from app.services import translation

NAME = "tarjima_qil"


def _find_document(
    db: Session, user: User, args: dict
) -> tuple[Document | None, str | None]:
    """Locate the document. Returns (document, error_text)."""
    raw_id = args.get("hujjat_id")
    if raw_id not in (None, ""):
        try:
            doc_id = int(raw_id)
        except (TypeError, ValueError):
            return None, f"'hujjat_id' butun son bo'lishi kerak, kelgani: {raw_id!r}."
        if db.get(Document, doc_id) is None:
            return None, f"{doc_id}-raqamli hujjat topilmadi."
        document = documents_service.get_visible_document(db, user, doc_id)
        if document is None:
            return None, (
                "Ruxsat yo'q: bu hujjat sizning rolingiz uchun yopiq. "
                "Foydalanuvchiga hujjat mazmunini aytma."
            )
        return document, None

    name = str(args.get("nom") or "").strip()
    if not name:
        return None, "Hujjatni aniqlash uchun 'nom' yoki 'hujjat_id' bering."

    document = documents_service.find_visible_document_by_title(db, user, name)
    if document is None:
        return None, f"'{name}' bo'yicha sizga ochiq hujjatlardan mos hujjat topilmadi."
    return document, None


def handler(db: Session, user: User, args: dict) -> ToolResult:
    document, error = _find_document(db, user, args)
    if error or document is None:
        return ToolResult(text=error or "Hujjat topilmadi.", ok=False)

    requested = args.get("til") or user.language or translation.DEFAULT_LANGUAGE
    try:
        language = translation.normalize_language(str(requested))
    except translation.UnsupportedLanguageError:
        return ToolResult(
            text=(
                f"'{requested}' tili qo'llab-quvvatlanmaydi. Mavjud tillar: "
                f"{', '.join(translation.SUPPORTED_LANGUAGES)}."
            ),
            ok=False,
        )

    try:
        result = translation.translate_document(db, document, user, language)
    except translation.EmptyDocumentError:
        return ToolResult(
            text=f"'{document.title}' hujjatining matni topilmadi.", ok=False
        )

    return ToolResult(
        text=translation.format_pairs_for_tool(result),
        sources=[result.source],
    )


register(
    Tool(
        name=NAME,
        description=(
            "Hujjatni boshqa tilga tarjima qiladi va originalni yonida saqlaydi "
            "(paragraf-paragraf). Hujjat nomi yoki id raqami bo'yicha topadi. "
            "'Bu maqolani o'zbekchaga o'gir', 'inglizcha adabiyotni tarjima qil' "
            "kabi so'rovlarda ishlat."
        ),
        parameters={
            "type": "object",
            "properties": {
                "nom": {
                    "type": "string",
                    "description": "Hujjat nomi yoki uning bir qismi.",
                },
                "hujjat_id": {
                    "type": "integer",
                    "description": "Hujjatning id raqami (qidiruv natijasidan olinadi).",
                },
                "til": {
                    "type": "string",
                    "enum": list(translation.SUPPORTED_LANGUAGES),
                    "description": (
                        "Maqsad tili kodi. Berilmasa foydalanuvchi tili ishlatiladi."
                    ),
                },
            },
            "required": [],
        },
        handler=handler,
        roles=ALL_ROLES,
    )
)
